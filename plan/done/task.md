...
- [x] **`HdrHistogram/HistogramBase.cs`** — Add `GetPercentileAtOrBelowValue(long value)` as a public instance method returning `double`, placed directly after `GetValueAtPercentile` (around line 388).
...
- [x] **`HdrHistogram/HistogramBase.cs`** — Add XML doc comment for `GetPercentileAtOrBelowValue` following the existing style (summary, `<param>`, `<returns>`).
...
- [x] **Empty histogram returns 0.0** *(criterion 5)*
...
- [x] **Value at or above `HighestTrackableValue` returns 100.0** *(criterion 4)*
...
- [x] **Basic percentile — known value set** *(criterion 3)*
...
- [x] **Return value is always in range `[0.0, 100.0]`** *(criterion 2)*
...
- [x] **Round-trip consistency with `GetValueAtPercentile`** *(criterion 6)*
...
- [x] **Monotonicity — non-decreasing percentiles for increasing values** *(criterion 7)*
...
- [x] **Single recorded value — queries below return 0.0, queries at or above return 100.0** *(criteria 3, 4, 5)*
...
- [x] **Boundary — query at `LowestTrackableValue`** *(criterion 3)*
